...
# Resolve the artifacts directory relative to this file so the env can be
# imported from anywhere (scripts/, notebooks, etc.)
import os
...
class SpacecraftEnv(gym.Env):
    def __init__(self):
        ...
        self.anomaly_detector = joblib.load(os.path.join(_ARTIFACTS_DIR, "isolation_forest.pkl"))
        ...
        # numbers and require the use of n closed intervals
    ...
    def _precompute_ephemeris(self):
        ...
        # Unpacking directly lets Pyright resolve the correct CartesianRepresentation types.
        ...
        # .T reshapes from (3, 11041) → (11041, 3)
        mars_pos = mars_pos_bary.xyz.value.T * 149597870.7  # type: ignore[union-attr]
        mars_vel = mars_vel_bary.xyz.value.T * 1731.4568    # type: ignore[union-attr]
